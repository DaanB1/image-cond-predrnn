from core.data_provider import kth_action, mnist, bair, eyetrack
...
def data_provider(dataset_name, train_data_paths, valid_data_paths, batch_size,
    ...
    if dataset_name == 'eyetrack':
        input_param = {'paths': valid_data_list,
                       ...
                       'input_data_type': 'float32',
                       'name': dataset_name + ' iterator'}
        input_handle = datasets_map[dataset_name]
        if is_training:
            train_input_handle = input_handle.get_train_input_handle(input_param)
            train_input_handle.begin(do_shuffle=True)
            test_input_handle = input_handle.get_test_input_handle(input_param)
            test_input_handle.begin(do_shuffle=False)
            return train_input_handle, test_input_handle
        else:
            test_input_handle = input_handle.get_test_input_handle(input_param)
