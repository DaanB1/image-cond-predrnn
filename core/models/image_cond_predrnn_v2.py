...
class RNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        if self.configs.conv_on_input == 1:
            ...
            self.resnet.fc = nn.Linear(2048, num_hidden[0] * self.rnn_height * self.rnn_width)
    ...
    def forward(self, all_frames, mask_true):
        ...
        for i in range(self.num_layers):
            zeros = torch.zeros(
                [self.configs.batch_size, self.num_hidden[i], self.rnn_height, self.rnn_width]).to(self.configs.device)
            h_t.append(zeros)
            c_t.append(zeros)
        ...
        decouple_loss = []
        memory = torch.zeros([self.configs.batch_size, self.num_hidden[0], self.rnn_height, self.rnn_width]).to(self.configs.device)
        ...
        for t in range(self.configs.total_length - 1):
            ...
            action = self.resnet(action)
            action = action.view(-1, self.num_hidden[0], self.rnn_height, self.rnn_width)
